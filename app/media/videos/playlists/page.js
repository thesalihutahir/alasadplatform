...
import { ListVideo, PlayCircle, Loader2, Layers, ChevronRight, Search, X, ArrowRight, Film, ChevronDown, ChevronUp } from 'lucide-react';
            ...
            <main className="flex-grow pb-24">
                ...
                {loading ? (
                    ...
                    <div className="max-w-[1400px] mx-auto px-4 md:px-8 lg:px-12">
                        ...
                        {/* 2. REFINED FILTER BAR (Matched Video Library Style) */}
                        <div className="flex justify-center mb-12">
                            <div className="flex flex-row gap-2 overflow-x-auto pb-2 scrollbar-hide px-2 w-full md:w-auto justify-start md:justify-center">
                                {filters.map((filter) => (
                                    <button 
                                        key={filter}
                                        onClick={() => {
                                            setActiveFilter(filter);
                                            setVisibleCount(15); // Reset count on filter change
                                        }}
                                    ...
                                    >
                                        {filter}
                                        {activeFilter === filter && <div className="w-1.5 h-1.5 rounded-full bg-brand-gold ml-2 hidden lg:block"></div>}
                                    </button>
                                ))}
                            </div>
                        </div>
